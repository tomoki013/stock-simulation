(() => {
    ...
    const $totalWrapper = document.querySelector('.total_wrapper');
    ...
    // 会社情報
    const companies = [
        { name: "Company A", stockPrice: 1000, shares: 0, buyPrice: 0, totalStockValueAtBuy: 0 },
        { name: "Company B", stockPrice: 1500, shares: 0, buyPrice: 0, totalStockValueAtBuy: 0 },
        { name: "Company C", stockPrice: 1800, shares: 0, buyPrice: 0, totalStockValueAtBuy: 0 },
        { name: "Company D", stockPrice: 2000, shares: 0, buyPrice: 0, totalStockValueAtBuy: 0 },
    ];
    ...
    function initializeCompanies() {
        ...
        const totalWrapperHeight = $totalWrapper.offsetHeight;
        const bodyMarginBottom = totalWrapperHeight + 10;
        document.body.style.marginBottom = `${bodyMarginBottom}px`;
    ...
    function advanceTime() {
        ...
        if (currentHour === closeHour && currentMinute === closeMinute) {
            ...
            stopStockPriceUpdates();
            showPopup(`
                場外時間です。場内時間に戻るまで株価は変動しません。
                場内時間: 09:00 ~ ${closeHour}:${closeMinute}
                `);
    ...
    function updateStockPrice(company, index) {
        ...
        document.getElementById(`stockChangeDaily-${index}`).innerText = `株価増減: ${change >= 0 ? "+" : ""}${change.toLocaleString()}円`;
    ...
    // 利益を計算する関数
    function calculateProfit(company, amount) {
        return amount * (company.stockPrice - company.buyPrice);
    }
    ...
    function buyStock(index) {
        ...
        if (balance >= totalCost) {
            ...
            showPopup(`
                ${company.name}
                購入株数: ${amount} 株
                株価: ${company.stockPrice.toLocaleString()} 円
    ...
    function sellStock(index) {
        ...
        if (company.shares >= amount) {
            ...
            const profit = calculateProfit(company, amount);
            ...
            showPopup(`
                ${company.name}
                売却株数： ${amount} 株
                金額： ${totalRevenue.toLocaleString()} 円
                利益： ${profit >= 0 ? "+" : ""}${profit.toLocaleString()} 円`);
        } else {
    ...
    function updateTotalStockValue(company, index) {
        ...
        const amount = parseInt(document.getElementById(`shareAmount-${index}`).value);
        if (amount > 0) {
            const stockValueChange = calculateProfit(company, amount)
